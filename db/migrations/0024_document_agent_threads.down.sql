CREATE TABLE document_agent_threads_rollback_guard (
  allowed INTEGER NOT NULL CHECK (allowed = 0)
);

INSERT INTO document_agent_threads_rollback_guard (allowed)
SELECT 1 WHERE EXISTS (SELECT 1 FROM document_agent_conversations);

DROP TABLE document_agent_threads_rollback_guard;

DROP TRIGGER document_agent_conversations_thread_immutable;
DROP TRIGGER document_agent_conversation_requests_delete_forbidden;
DROP TRIGGER document_agent_conversation_requests_update_forbidden;
...
DROP TABLE document_agent_conversation_requests;
...
DROP INDEX document_agent_conversations_document;
...
CREATE TABLE document_agent_messages (
  ...
  actor_user_id TEXT,
  text TEXT NOT NULL CHECK (length(text) BETWEEN 1 AND 2000 AND text = trim(text)),
  model_id TEXT,
  runtime_version TEXT,
  created_at TEXT NOT NULL,
  UNIQUE (family_id, id),
  ...
  CONSTRAINT document_agent_message_role_shape CHECK (
    (role = 'user' AND actor_user_id IS NOT NULL AND model_id IS NULL AND runtime_version IS NULL)
    OR (
      role = 'assistant' AND actor_user_id IS NULL AND model_id IS NOT NULL
      AND length(model_id) BETWEEN 1 AND 100 AND model_id = trim(model_id)
      AND runtime_version IS NOT NULL AND length(runtime_version) BETWEEN 1 AND 100
      AND runtime_version = trim(runtime_version)
    )
  )
);
