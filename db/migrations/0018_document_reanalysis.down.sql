...
DROP TRIGGER processing_restart_requests_delete_forbidden;
DROP TRIGGER processing_restart_requests_update_forbidden;
DROP TRIGGER processing_restart_requests_pending_job_only;
DROP INDEX processing_restart_requests_document;
DROP TABLE processing_restart_requests;
...
ALTER TABLE document_intelligence_results
  RENAME TO document_intelligence_results_0018;
...
SELECT id, family_id, document_id, document_version_id, processing_job_id,
       provider, model_id, runtime_version, schema_version, category, title,
       document_date, confidence, created_at
  FROM document_intelligence_results_0018;

DROP TABLE document_intelligence_results_0018;
