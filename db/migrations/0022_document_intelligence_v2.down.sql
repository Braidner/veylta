CREATE TABLE document_intelligence_v2_rollback_guard (
  allowed INTEGER NOT NULL CHECK (allowed = 1)
);

INSERT INTO document_intelligence_v2_rollback_guard (allowed)
SELECT 0
 WHERE EXISTS (
   SELECT 1
     FROM document_intelligence_results
    WHERE schema_version = 'document-intelligence/v2'
 );

DROP TABLE document_intelligence_v2_rollback_guard;
...
ALTER TABLE document_intelligence_results
  RENAME TO document_intelligence_results_0022;
...
SELECT id, family_id, document_id, document_version_id, processing_job_id,
       provider, model_id, runtime_version, schema_version, category, title,
       document_date, confidence, created_at
  FROM document_intelligence_results_0022;

DROP TABLE document_intelligence_results_0022;
