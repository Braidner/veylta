...
ALTER TABLE document_intelligence_results
  RENAME TO document_intelligence_results_0017;
...
SELECT id, family_id, document_id, document_version_id, processing_job_id,
       provider, model_id, runtime_version, schema_version, category, title,
       document_date, confidence, created_at
  FROM document_intelligence_results_0017;

DROP TABLE document_intelligence_results_0017;
...
CREATE TABLE processing_restart_requests (
  id TEXT PRIMARY KEY,
  family_id TEXT NOT NULL REFERENCES families(id) ON DELETE RESTRICT,
...
CREATE INDEX processing_restart_requests_document
  ON processing_restart_requests (family_id, document_version_id, created_at DESC);

CREATE TRIGGER processing_restart_requests_pending_job_only
BEFORE INSERT ON processing_restart_requests
WHEN NOT EXISTS (
  SELECT 1
...
BEGIN
  SELECT RAISE(ABORT, 'processing restart requires a fresh pending job');
END;

CREATE TRIGGER processing_restart_requests_update_forbidden
BEFORE UPDATE ON processing_restart_requests
BEGIN
  SELECT RAISE(ABORT, 'processing restart requests are immutable');
END;

CREATE TRIGGER processing_restart_requests_delete_forbidden
BEFORE DELETE ON processing_restart_requests
BEGIN
  SELECT RAISE(ABORT, 'processing restart requests are immutable');
END;
