-- Back to the physician-only CHECK, the same way: the other assistants' conversations cannot
-- fit it, so their exchanges, requests, messages and conversations go first — a rollback of this
-- slice loses those dialogues, and nothing else.
...
DELETE FROM assistant_exchanges
 WHERE (family_id, conversation_id) IN (
   SELECT family_id, id FROM assistant_conversations WHERE assistant_id <> 'physician');
DELETE FROM assistant_message_requests
 WHERE (family_id, conversation_id) IN (
   SELECT family_id, id FROM assistant_conversations WHERE assistant_id <> 'physician');
DELETE FROM assistant_conversation_requests
 WHERE (family_id, conversation_id) IN (
   SELECT family_id, id FROM assistant_conversations WHERE assistant_id <> 'physician');
DELETE FROM assistant_messages
 WHERE (family_id, conversation_id) IN (
   SELECT family_id, id FROM assistant_conversations WHERE assistant_id <> 'physician');
DELETE FROM assistant_conversations WHERE assistant_id <> 'physician';

CREATE TABLE assistant_conversations_0034 (
  id TEXT PRIMARY KEY,
...
INSERT INTO assistant_conversations_0034 SELECT * FROM assistant_conversations;

CREATE TABLE assistant_messages_0034 (
  id TEXT PRIMARY KEY,
  family_id TEXT NOT NULL REFERENCES families(id) ON DELETE RESTRICT,
  ...
  FOREIGN KEY (family_id, conversation_id)
    REFERENCES assistant_conversations_0034(family_id, id)
    ON DELETE RESTRICT,
...
INSERT INTO assistant_messages_0034 SELECT * FROM assistant_messages;

CREATE TABLE assistant_conversation_requests_0034 (
  id TEXT PRIMARY KEY,
  family_id TEXT NOT NULL REFERENCES families(id) ON DELETE RESTRICT,
...
INSERT INTO assistant_conversation_requests_0034 SELECT * FROM assistant_conversation_requests;

CREATE TABLE assistant_message_requests_0034 (
  id TEXT PRIMARY KEY,
  family_id TEXT NOT NULL REFERENCES families(id) ON DELETE RESTRICT,
  actor_user_id TEXT NOT NULL,
  conversation_id TEXT NOT NULL,
  ...
  FOREIGN KEY (family_id, conversation_id, user_message_id)
    REFERENCES assistant_messages_0034(family_id, conversation_id, id)
    ON DELETE RESTRICT,
  FOREIGN KEY (family_id, conversation_id, assistant_message_id)
    REFERENCES assistant_messages_0034(family_id, conversation_id, id)
    ON DELETE RESTRICT
);
INSERT INTO assistant_message_requests_0034 SELECT * FROM assistant_message_requests;

CREATE TABLE assistant_exchanges_0034 (
  id TEXT PRIMARY KEY,
  family_id TEXT NOT NULL REFERENCES families(id) ON DELETE RESTRICT,
  conversation_id TEXT NOT NULL,
...
INSERT INTO assistant_exchanges_0034 SELECT * FROM assistant_exchanges;
...
ALTER TABLE assistant_conversations_0034 RENAME TO assistant_conversations;
ALTER TABLE assistant_messages_0034 RENAME TO assistant_messages;
ALTER TABLE assistant_conversation_requests_0034 RENAME TO assistant_conversation_requests;
ALTER TABLE assistant_message_requests_0034 RENAME TO assistant_message_requests;
ALTER TABLE assistant_exchanges_0034 RENAME TO assistant_exchanges;
