# Delivered scope and operational detail

The README keeps to what a newcomer needs. This page holds the running record the README used to
carry: the deployment model, every delivered slice, the executable architecture summary and the
operational detail of the current build. It is descriptive, not a roadmap; see
[slices](slices.md) for the plan and [architecture](architecture.md) for the design.

## Deployment model: a home health-care PWA
...
[ADR 0007](adr/0007-home-server-pwa-and-codex-runtime.md).
...
See [product](product.md), [architecture](architecture.md),
[threat model](threat-model.md), [API](api.md), [ER model](er-model.md),
[slice plan](slices.md), and the
[first-slice acceptance evidence](first-slice-acceptance.md).
