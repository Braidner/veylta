...
| Malformed PDF/image/parser/renderer exploit | Code execution, crash, or data disclosure | Bounded PDF.js text-layer extraction; for text-layer-missing PDFs and direct images, rasterisation only — nothing local recognizes text — bounded by `MAX_DOCUMENT_IMAGE_SOURCE_BYTES` (32 MiB, deliberately far below the 100 MB a household may store), at most `MAXIMUM_DOCUMENT_IMAGE_PAGES` (3) pages, 2 million pixels/page, 4 million pixels total, timeout, strict synthetic grammar, controlled byte snapshot, security updates, and adversarial tests. Direct PNG/JPEG has exact signature plus a 128 KiB bounded header pixel-cap preflight before decode. Rendered pages leave the machine as `--image` attachments Codex transcribes itself. Process isolation remains required before real data | Bounded subset in first slice; isolation hardening before real data |
...
| Unsafe medical output (care plan) | Diagnosis/treatment harm or missed urgency | Codex can only select one bounded draft per existing lane; deterministic copy contains no diagnosis, treatment, triage, prescription, or urgency. Each item remains `proposed`, with immutable run/summary/model/rule/source/context provenance and explicit acceptance | Codex household plan |
| Unsafe medical output (assistant) | A second opinion harms, over-reassures, or misses urgency | The physician assistant answers only in a closed schema of typed blocks; the parser verifies every block against the confirmed evidence and drops unbound ones, refuses any medication dose, refuses general text that quotes the person's values, and requires an urgency tier that is rendered as fixed copy and never lowered; every hypothesis and treatment option must name a confirming specialty; a second independent `codex exec` refutes the answer — contradicted/unsafe blocks drop, overreach lowers confidence, the higher urgency wins; a fully refuted answer is refused with a closed reason; nothing is interpreted before sex and birth year are recorded; accepting a referral is a human action creating one `proposed`-lane item | Physician assistant (docs/assistants.md, slice 1) |
| Assistant impersonation | Conversational UI is mistaken for a doctor, nutritionist, or trainer | Role labels `ИИ-врач · второе мнение`, `ИИ-нутрициолог`, `ИИ-тренер` say what they are; every surface states that the output is a recommendation to confirm with a clinician; landing messages derive from server-authoritative state; never an opaque health score | Profile landing UI, assistant workspace |
| Консилиум drift | A synthesis softens an alarm or invents agreement between personas | The panel is decided by a deterministic code→specialty table, never by the model; every persona's opinion is parsed and refuted on its own; the synthesis parser keeps only agreement notes that name invited specialties; the checker sees the opinions when refuting the synthesis; the UI shows every opinion beside the synthesis; the synthesis prompt requires the highest urgency and the applied checker urgency can only raise it | Консилиум (docs/assistants.md, slice 2) |
| Assistant egress without consent | Confirmed values and profile sent to the provider unnoticed | Per-conversation acknowledgement of a disclosure that lists exactly what the evidence loader sends (confirmed observations with printed ranges, medical profile, care plan); the server refuses a message before it (409); no document, page, file, path, key or family/profile ID travels; features, shell, browser and web search disabled; the raw exchange is journaled for the owner only, never audited or logged | Physician assistant |
