...
| Unsafe medical output | Diagnosis/treatment harm or missed urgency | The delivered summary is a closed, non-clinical evidence snapshot. The household plan labels person-authored decisions separately; a future Codex item is stored only as `proposed`, bound to immutable evidence/rule/missing-context provenance, and needs explicit acceptance. No risk/red-flag/diagnosis/treatment field exists. Clinical recommendations still require a separately reviewed safety boundary | Household plan foundation; before Codex or clinical recommendation features |
