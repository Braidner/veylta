// The regimen rooms' scripted answers: the nutritionist's plan and the trainer's programme, both
// keyed on the first block kind of the schema they were asked for.

export function refsOf(prompt: string) {
  const ids = [...prompt.matchAll(/"observationId":"([0-9a-f-]{36})"/g)].map((match) => match[1]);
  return ids[0] === undefined ? [] : [{ observationId: ids[0] }];
}

export function notReady(): unknown {
  return {
...
export function nutritionistOutput(prompt: string): unknown {
...
/** The trainer's scripted programme: an assessment, a walk within clearance, strength that needs it, a stop rule. */
export function trainerOutput(prompt: string): unknown {
  const ref = refsOf(prompt);
  if (!prompt.includes('"interpretationReady":true') || ref.length === 0) return notReady();
