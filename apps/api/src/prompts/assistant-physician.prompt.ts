...
const rules = [
  ...
  "Clinician records (clinicianRecords) are the person's own doctors' statements — diagnoses, prescriptions as written, referrals, follow-ups, procedures, findings — confirmed by the person from their documents. When the person asks to compare («сверка», «сверь», «согласны ли вы с врачом») or when a record bears on your read, add one clinician_check block per record: theirs.recordId names the record; claim is agree when your read of the evidence supports it, differs when it does not, cannot_assess when the evidence does not allow a view; ours is your own position in one sentence, why cites the observations, confirmWith is the specialty to bring it to. Never call the clinician wrong and never rate them: a difference is a question for the next visit. Do not repeat a prescription's dose as your own recommendation.",
